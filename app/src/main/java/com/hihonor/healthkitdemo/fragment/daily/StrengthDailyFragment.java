 ...
 * @since 2024-08-01
 */
public class StrengthDailyFragment extends BaseFragment {
    private static final String TAG = "StrengthDailyFragment";
    ...
    private TextView mTvThreeResultUnit;
    ...
    protected void initView(View view) {
        ...
        mTvTitle.setText("今日活动总时长");
        mTvEveryMinuteTitle.setText("每分钟锻炼类型");
        ...
        mTvFiveTitle = view.findViewById(R.id.tv_five_title);
        mTvFiveTitle.setText(R.string.sport_natation);
        mTvThreeResultUnit = view.findViewById(R.id.tv_three_result_unit);
        ...
        getStrengthStatisticData();
        getEveryMinuteData();
    }

    private void getStrengthStatisticData() {
        long currentTime = System.currentTimeMillis();
        long startTime = TimeUtils.getMinTimeMillis(currentTime);
        long endTime = TimeUtils.getMaxTimeMillis(currentTime);
        Observable.create(emitter -> {
            QueryRequest queryRequestStep = new QueryRequest(DataType.SAMPLE_STRENGTH_STATISTIC, startTime, endTime);
            HealthKit.getDataStoreClient(getActivity().getApplicationContext())
    ...
    private void setTextViewDate(SampleData sampleData) {
        ...
        if (sampleData.getInteger(StrengthStatisticField.FIELD_RUN_DURATION_NAME) != null) {
            mTvOneResultUnit
                .setText(sampleData.getInteger(StrengthStatisticField.FIELD_RUN_DURATION_NAME).toString() + " 秒");
        } else {
            mTvOneResultUnit.setText(0 + " 秒");
        }
        if (sampleData.getInteger(StrengthStatisticField.FIELD_WALK_DURATION_NAME) != null) {
            mTvTwoResultUnit
                .setText(sampleData.getInteger(StrengthStatisticField.FIELD_WALK_DURATION_NAME).toString() + " 秒");
        } else {
            mTvTwoResultUnit.setText(0 + " 秒");
        }
        if (sampleData.getInteger(StrengthStatisticField.FIELD_RIDE_DURATION_NAME) != null) {
            mTvThreeResultUnit
                .setText(sampleData.getInteger(StrengthStatisticField.FIELD_RIDE_DURATION_NAME).toString() + " 秒");
        } else {
            mTvThreeResultUnit.setText(0 + " 秒");
        }
        if (sampleData.getInteger(StrengthStatisticField.FIELD_CLIMB_DURATION_NAME) != null) {
            mTvFourResultUnit
                .setText(sampleData.getInteger(StrengthStatisticField.FIELD_CLIMB_DURATION_NAME).toString() + " 秒");
        } else {
            mTvFourResultUnit.setText(0 + " 秒");
        }
        if (sampleData.getInteger(StrengthStatisticField.FIELD_SWIM_DURATION_NAME) != null) {
            mTvFiveResultUnit
                .setText(sampleData.getInteger(StrengthStatisticField.FIELD_SWIM_DURATION_NAME).toString() + " 秒");
        } else {
            mTvFiveResultUnit.setText(0 + " 秒");
        }
    }
    ...
    private void getEveryMinuteData() {
        ...
        long startTime = TimeUtils.getMinTimeMillis(currentTime);
        long endTime = TimeUtils.getMaxTimeMillis(currentTime);
        QueryRequest queryRequestStep = new QueryRequest(DataType.SAMPLE_STRENGTH, startTime, endTime);
        Observable.create(emitter -> HealthKit.getDataStoreClient(getActivity().getApplicationContext())
            .querySampleData(Utils.getHonorSignInAccount(), queryRequestStep)
            .addOnSuccessListener(sampleDataQueryResponse -> {
                List<SampleData> dataList = sampleDataQueryResponse.getDataList();
    ...
    private void insertEveryMinuteData() {
        ...
        sampleData.setDataType(DataType.SAMPLE_STRENGTH);
        ...
        sampleData.putInteger(StrengthField.FIELD_MOTION_TYPE_NAME, 1);
